...
st_leaf = st.one_of(
    ...
    st.floats(allow_nan=False, allow_infinity=False, width=64).map(str),
...
def valid_document(draw):
    ...
    doc = []
    # Mix simple and dotted keys without prefix overlap
    for k in keys:
        if draw(st.booleans()):
...
@st.composite
def _malformed_cannot_extend_array(draw):
    return "a = [1]\na = [2]"
...
@st.composite
def _malformed_dotted_key_type(draw):
    k = draw(_bare_name())
    return f"{k} = 1\n[{k}.a]"

@st.composite
def _malformed_bad_control_char_comment(draw):
    return "x = 1 # \u0000"
...
def _malformed_document():
    return st.one_of(
        ...
        _malformed_cannot_extend_array(),
        _malformed_bad_escape(),
        _malformed_dotted_key_type(),
        _malformed_bad_control_char_comment(),
