...
def _gen_valid_string():
    # Helper to generate varied string types and valid escapes
    escapes = ["\\b", "\\f", "\\n", "\\r", "\\t", "\\e", "\\x20", "\\u1234", "\\U0001F600"]
    return st.one_of(
        st.text(alphabet="abcdefghijklmnopqrstuvwxyz0123456789 ", min_size=1, max_size=10).map(lambda s: f'"{s}"'),
        st.text(alphabet="abcdefghijklmnopqrstuvwxyz0123456789 ", min_size=1, max_size=10).map(lambda s: f"'{s}'"),
        st.text(alphabet="abc", min_size=1, max_size=5).map(lambda s: f'"""{s}"""'),
    )
...
st_leaf = st.one_of(
    ...
    st.floats(allow_infinity=True, allow_nan=True).filter(lambda f: not math.isnan(f)).map(str),
...
def _malformed_duplicate_key(draw):
    k = draw(_bare_name())
    return f"{k} = {draw(st_leaf)}\n{k} = {draw(st_leaf)}"

@st.composite
def _malformed_dotted_key_dup(draw):
    k = draw(_bare_name())
    return f"{k}.a = 1\n{k}.a = 2"
...
@st.composite
def _malformed_array_table_conflict(draw):
    k = draw(_bare_name())
    return f"[[{k}]]\n[{k}]"
...
@st.composite
def _malformed_long_dotted_key(draw):
    k = "a." * 15 + "b"
    return f"{k} = 1"

@st.composite
def _malformed_deep_nesting(draw):
    return "[" * 100 + "]" * 100

@st.composite
def _malformed_unterminated_string(draw):
    return 'x = "foo'

@st.composite
def _malformed_unexpected_newline(draw):
    return 'x = \n 1'

@st.composite
def _malformed_utf8_error(draw):
    # Represent invalid utf8 sequence via Python surrogates
    return 'x = "\ud800"'
...
def _malformed_document():
    return st.one_of(
        ...
        _malformed_unterminated_string(),
        _malformed_unexpected_newline(),
        _malformed_utf8_error()
    )
