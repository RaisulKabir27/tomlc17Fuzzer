...
def valid_document(draw):
    ...
    n_tables = draw(st.integers(min_value=0, max_value=2))
...
MAX_DEPTH = 16             # ceiling on drawn container nesting depth
# ---------------------------------------------------------------------------
...
def _malformed_long_dotted_key(draw):
    keys = [draw(_bare_name()) for _ in range(12)]
    return f"{'.'.join(keys)} = 1"
